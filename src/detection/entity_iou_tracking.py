...
def entity_iou_tracking (session: Session, images: list[Image], trackingDuration: dt.timedelta = dt.timedelta(seconds=1)):
    tracks: dict[Entity, Track] = dict()

    for image in images:
        for (entity, track) in tracks.items():
            if image.datetime - track.lastSeen < trackingDuration and len(track.instances) > 1:
                for instance in track.instances:
                    instance.direction_lr = 1 if track.instances[0].center()[0] < track.instances[-1].center()[0] else -1
        tracks = dict(filter(lambda track: image.datetime - track[1].lastSeen < trackingDuration, tracks.items()))

        for instance in image.get_instances(session):
            bestEntity: Entity | None = None
            bestIou: float = 0
            for (entity, track) in tracks.items():
                if track.instances[-1].type_id != instance.type_id or track.lastSeen == image.datetime:
                    continue
            ...
            if bestEntity is not None:
                ...
                tracks[instance.entity] = Track(image.datetime, [instance])
