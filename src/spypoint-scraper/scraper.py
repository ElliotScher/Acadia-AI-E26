...
parser.add_argument(
    "-m", "--max", help="Maximum number of photos to download", type=int
)
...
if args.camera[0] == "list":
    ...
    camera_list_request = requests.get(
...
while end_date != "done":
    photo_list_request = requests.post(
    ...
    for photo in photo_list["photos"]:
        ...
        timestamp = datetime.datetime.strptime(
            photo["originDate"][0:19], "%Y-%m-%dT%H:%M:%S"
        )
        path = os.path.join(args.destination[0], date, time + ".jpg")

        try:
            os.mkdir(os.path.join(args.destination[0], date))
        ...
        photo_request = requests.get(
            "https://%s/%s" % (photo[args.size]["host"], photo[args.size]["path"])
        )
        ...
        print(
            f"\033[K{downloaded}/{str(total) if total % 100 != 0 else str(total) + "+"} photos...",
            end="\r",
        )
