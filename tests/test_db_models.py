...
def test_session():
    ...
    # create testing data
    image1 = Image(path="/img1.jpg", datetime=datetime.fromisoformat("2026-06-26T12:14:02Z"))
    image2 = Image(path="/img2.jpg", datetime=datetime.fromisoformat("2026-06-26T12:14:04Z"))
    image3 = Image(path="/img3.jpg", datetime=datetime.fromisoformat("2026-06-26T12:15:37Z"))
    image4 = Image(path="/img4.jpg", datetime=datetime.fromisoformat("2026-06-26T12:15:38Z"))
    ...
    instance1 = Instance(image=image1, entity=entity1, type_id=0, x=0, y=0, width=0, height=0)
    instance2 = Instance(image=image1, entity=entity2, type_id=0, x=0, y=0, width=0, height=0)
    instance3 = Instance(image=image2, entity=entity1, type_id=0, x=0, y=0, width=0, height=0)
    instance4 = Instance(image=image2, entity=entity2, type_id=0, x=0, y=0, width=0, height=0)
    instance5 = Instance(image=image3, entity=entity3, type_id=1, x=0, y=0, width=0, height=0)
    instance6 = Instance(image=image3, entity=entity4, type_id=1, x=0, y=0, width=0, height=0)
    instance7 = Instance(image=image4, entity=entity5, type_id=0, x=0, y=0, width=0, height=0)
    instance8 = Instance(image=image4, entity=entity3, type_id=1, x=0, y=0, width=0, height=0)
    instance9 = Instance(image=image4, entity=entity4, type_id=1, x=0, y=0, width=0, height=0)

    # add testing data
    with Session(db) as session:
        session.add_all((image1, image2, image3, image4))
        session.add_all((entity1, entity2, entity3, entity4, entity5))
        session.add_all((instance1, instance2, instance3, instance4, instance5, instance6, instance7, instance8, instance9))
        session.commit()
...
def test_delete_entity(test_session: Session):
    ...
    assert not test_session.query(exists().where(Instance.entity_id == entity1.id)).scalar()

def test_delete_instance(test_session: Session):
    test_session.delete(instance1)
    test_session.commit()
   
    assert not test_session.query(exists().where(and_(Instance.entity_id == instance1.entity_id, Instance.image_id == instance1.image_id))).scalar()
    ...
    assert not test_session.query(exists().where(and_(Instance.entity_id == instance3.entity_id, Instance.image_id == instance3.image_id))).scalar()
    assert not test_session.query(exists().where(Instance.entity_id == entity1_id)).scalar()
...
def test_delete_image(test_session: Session):
    ...
    assert not test_session.query(exists().where(Instance.image_id == image1.id)).scalar()

    test_session.delete(image2)
    test_session.commit()

    assert not test_session.query(exists().where(Image.id == image2.id)).scalar()
    assert not test_session.query(exists().where(Instance.image_id == image2.id)).scalar()
    assert not test_session.query(exists().where(Entity.cluster == 1)).scalar()

def test_create(test_session: Session):
    image5 = Image(path="/img5.jpg", datetime=datetime.fromisoformat("2026-06-26T12:16:27Z"))
    ...
    entity6 = Entity()
    instance10 = Instance(image=image5, entity=entity6, type_id=0, x=0, y=0, width=0, height=0)

    test_session.add_all((entity6, instance10))
    test_session.commit()

    assert test_session.query(exists().where(Entity.id == entity6.id)).scalar()
    assert test_session.query(exists().where(and_(Instance.entity_id == entity6.id, Instance.image_id == image5.id))).scalar()
...
def test_transfer_entity(test_session: Session):
    ...
    assert not test_session.query(exists().where(Instance.entity_id == entity6_id)).scalar()
